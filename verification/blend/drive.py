...
Run a blend-measurement matrix through a running ComfyUI server.
    ...
    python verification/blend/drive.py {sd15,h3} --label NAME [--strengths 0.25,0.5]
        [--street-strengths 0.5] [--jump 0.5] [--seeds 8888,1234] [--dry-run]
...
The server has to have the pack loaded, SD 1.5 or MiniMax H3 installed under the
file names below, and nothing else queued: interleaving models makes it swap them
for every prompt.
"""
import argparse
import json
...
def sd15_graph(prefix, seed, strength, travel, phase, scale, size):
    s = dict(common.shader_inputs(seed, strength, travel, phase, scale), steps=20, cfg=7.0,
...
def h3_graph(prefix, seed, strength, travel, phase, scale, size):
    s = dict(common.shader_inputs(seed, strength, travel, phase, scale), steps=8, cfg=1.0,
...
GRAPHS = {"sd15": (sd15_graph, PORTRAIT), "h3": (h3_graph, FORGE)}
...
def plan(model, strengths, street_strengths, jump, seeds):
    runs = []
    for seed in seeds:
        for k in sorted(set([0.0] + strengths)):
            runs.append((seed, k, "walk", *common.BASE))
        for k in jump:
            runs.append((seed, k, "jump", *common.BASE))
        for k in street_strengths:
            if k not in strengths:
                runs.append((seed, k, "walk", *common.BASE))
            for phase, scale in common.VARIANTS:
                runs.append((seed, k, "walk", phase, scale))
    return runs
...
def main():
    ...
    runs = plan(args.model, floats(args.strengths), floats(args.street_strengths), floats(args.jump), seeds)
    ...
    for run in todo:
        ...
        seed, strength, travel, phase, scale = run
        row = dict(name=name, model=args.model, seed=seed, strength=strength, travel=travel, phase=phase,
                   scale=scale, prompt=prompt, **size, latent=files.get("latent"), image=files.get("image"),
                   video=files.get("video"), seconds=round(time.time() - started, 1))
