    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "shader_type": (["domain_warp", "tensor_field", "curl_noise", "temporal_coherent"], {"default": "domain_warp", "tooltip": "Which noise pattern to blend in; each has its own character when it shows. domain_warp: flowing, intricate distortions, the even-handed default. tensor_field: structured and directional. curl_noise: smooth fluid motion, and the one that shows soonest, at about half the strength of the others. temporal_coherent: 4D simplex with time as a real axis, built for smooth animation; on MiniMax H3 it shows sooner than domain_warp, as a dot-grid pattern past about 0.5. The live preview only draws the first three; picking temporal_coherent leaves the preview on its last pattern, which does not affect sampling."}),
